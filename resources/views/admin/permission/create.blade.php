...
<div class="card card-secondary card-outline">
    <form class="needs-validation" action="{{route('admin.permission.store')}}" method="post" novalidate>
        ...
        <div class="card-body">
            <div class="row">
                <x-input class="col-md-6" name="full_name" label="Nome da Permissão" value="{{old('full_name')}}" required />
                <x-input class="col-md-6" name="description" label="Descrição" value="{{old('description')}}" required />
            </div>
        </div>
