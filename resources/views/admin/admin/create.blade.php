...
<div class="card card-secondary card-outline">
  <form class="needs-validation" action="{{route('admin.admin.store')}}" method="post" novalidate>
    ...
    <div class="card-body">
      <div class="row">
        <x-input class="col-md-6" name="full_name" label="Nome Completo" value="{{old('full_name')}}" required />
        <x-input class="col-md-6" name="email" label="E-mail" type="email" placeholder="exemplo@exemplo" value="{{old('email')}}" required />
      </div>
      <div class="row">
        <x-input class="col-md-6" label="CPF" placeholder="SOMENTE NÚMEROS" data-mask="000.000.000-00" value="{{old('cpf')}}" required />
        <x-input class="col-md-6" name="birthday" label="Data de Aniversário" type="date" value="{{old('birthday')}}" required />
      </div>
      <div class="row">
